...
def test_selftest_runner_jobs_contract() -> None:
    ...
    assert "'[\"3.12\"]'" in python_versions_expr, (
    ...
    assert strategy.get("matrix") == "${{ fromJson(needs.select-scenarios.outputs.matrix) }}", (
    ...
    assert env.get("SCENARIO_LIST") == "${{ needs.select-scenarios.outputs.scenario_list }}", (
    ...
    assert "'[\"3.12\"]'" in aggregate_python, (
...
def test_selftest_runner_publish_job_contract() -> None:
    ...
    assert cache_step.get("uses") == "./.github/actions/artifact-cache", (
        "Self-test report cache should use the shared artifact-cache action."
    )
    assert cache_step.get("if") == "${{ env.ENABLE_HISTORY == 'true' && env.RUN_ID != '' }}", (
        "Cache step must guard on enable_history input and aggregate run id."
    )
    cache_with = cache_step.get("with", {})
    assert cache_with.get("cache-key-base") == "selftest-reusable-ci-report-${{ env.RUN_ID }}", (
        "Cache key base should stay stable for selftest report reuse."
    )
    assert cache_with.get("window-resolution") == "daily", (
        "Selftest report cache should use a daily run window."
    )
    assert cache_with.get("producer-run-id") == "${{ env.RUN_ID }}", (
...
def test_selftest_triggers_are_manual_only() -> None:
    ...
    for workflow_file in selftest_files:
        ...
        assert trigger_keys <= allowed_for_file, (
