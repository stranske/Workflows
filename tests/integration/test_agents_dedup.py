...
def _build_duplicate_comment(duplicates: list[dict]) -> str | None:
    if not duplicates:
        return None
    ...
    for duplicate in duplicates:
        lines.append(
            f"- **#{duplicate['number']}** - "
            f"[{duplicate['title']}]({duplicate['url']}) "
            f"({duplicate['score']} similarity)"
        )
...
def _simulate_agents_dedup_workflow(
    ...
    *,
    ...
    store = issue_dedup.IssueVectorStore(store=_FakeStore(results), provider="fake", model="fake", issues=open_issues)
    query = f"{new_title}\n\n{new_body}"
    ...
    return {
        ...
        "comment": _build_duplicate_comment(duplicates),
    }
