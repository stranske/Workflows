...
def test_assertion_replacement_removal_is_detected() -> None:
    ...
    other_hunk = f"@@ -1 +0 @@\n-        {old}\n@@ -9 +9 @@\n+        {new}\n"
    assert list(deliberate_break._assertion_diff_lines(other_hunk)) == [f"-        {old}"]
    extra_removal = diff + "@@ -12 +0 @@\n-        assert unrelated_condition\n"
    assert list(deliberate_break._assertion_diff_lines(extra_removal)) == [
        f"-        {old}",
        "-        assert unrelated_condition",
    ]
    duplicate_removal = f"@@ -1,2 +1 @@\n-        {old}\n-        {old}\n+        {new}\n"
    assert list(deliberate_break._assertion_diff_lines(duplicate_removal)) == [
        f"-        {old}",
        f"-        {old}",
    ]


def test_pr_body_issue_marker_cannot_exempt_removed_assertion(monkeypatch, tmp_path) -> None:
    old = "assert validate_store(without_page).valid  # validator allows document-only citations"
    new = "assert not validate_store(without_page).valid"
