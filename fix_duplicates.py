...
def remove_duplicate_app_tokens(filepath):
    ...
    while i < len(lines):
        ...
        if '- name: Mint GitHub App Token' in line:
            ...
            j = i + 1
            step_lines = [line]
            while j < len(lines) and not (lines[j].strip().startswith('- name:') or re.match(r'^  \w+:', lines[j])):
            ...
            if j < len(lines) and '- name: Mint GitHub App Token' in lines[j]:
                # Skip the duplicate
                k = j + 1
                while k < len(lines) and not (lines[k].strip().startswith('- name:') or re.match(r'^  \w+:', lines[k])):
                    k += 1
