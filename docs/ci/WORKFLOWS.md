...
* [`agents-verifier.yml`](../../.github/workflows/agents-verifier.yml) runs on merged PRs (or pushes to the default branch) to assemble acceptance/task context, execute Codex in verifier mode (checkbox, evaluate, or compare), and post a verdict. In `compare` mode, two LLM providers (gpt-5.2 + claude-sonnet-4-5) evaluate independently with unanimous-PASS consensus. On CONCERNS or FAIL, `verify:create-new-pr` triggers `agents-verify-to-new-pr.yml` which uses a 4-round LLM pipeline to generate a follow-up issue. Follow-up chain depth should not exceed 2; automated enforcement is pending. See [`docs/analysis/verify-compare-40pr-evaluation-feb-2026.md`](../analysis/verify-compare-40pr-evaluation-feb-2026.md) for the Feb 2026 evaluation (40-PR sample, 35% first-fix rate, 2.7 avg chain depth).
