...
* [`agents-autofix-dispatcher.yml`](../../.github/workflows/agents-autofix-dispatcher.yml) listens for legacy `autofix_gate_failure` repository dispatches (sent by Gate), validates the payload, and records that `agents-81-gate-followups.yml` owns the consolidated Gate follow-up repair path.
