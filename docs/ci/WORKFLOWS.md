...
* [`agents-verifier.yml`](../../.github/workflows/agents-verifier.yml) runs on merged PRs (or pushes to the default branch) to assemble acceptance/task context, execute Codex in verifier mode (checkbox, evaluate, or compare), and open a follow-up issue when the verdict is CONCERNS or FAIL. Follow-up chain depth is capped at 2.
