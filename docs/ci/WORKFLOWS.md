...
* [`agents-keepalive-sweep.yml`](../../.github/workflows/agents-keepalive-sweep.yml) runs a periodic level-based resync (#2267): it dispatches the keepalive loop for open agent PRs so silent zero-commit stalls resurface instead of sitting idle until the next Gate event.
