...
| **Reusable Backplane Conformance** (`reusable-backplane-conformance.yml`, reusable bucket) | `workflow_call` | Validate a participating repo's emitted run-contract/v1 envelope (producer/bridge) or ingested satellite object (consumer) against the canonical Workflows-owned schemas plus the opt-in participant registry. The caller repo identity is bound to the registry lookup. The Inv-Man opt-in policy additionally verifies evidence-object files, manifest hashes, and two-way evidence-reference closure. No-op for non-participants. | ⚪ Reusable (opt-in) | [Backplane conformance runs](https://github.com/stranske/Workflows/actions/workflows/reusable-backplane-conformance.yml) |
...
| **Maint 69 Sync Integration Repo** (`maint-69-sync-integration-repo.yml`, maintenance bucket) | `push` (templates), `workflow_dispatch` | Sync integration-repo templates to Workflows-Integration-Tests repository. Resolves drift detected by Health 67. Supports dry-run mode; failed-sync alerts close automatically after the next successful non-dry run. | ⚪ Automatic/manual | [Integration sync runs](https://github.com/stranske/Workflows/actions/workflows/maint-69-sync-integration-repo.yml) |
...
| **Maint 83 Bootstrap Consumer** (`maint-83-bootstrap-consumer.yml`, maintenance bucket) | `workflow_dispatch` | Applies the manual GitHub-settings bootstrap a freshly-registered consumer needs: workflow permissions, required variables, the automation-bot collaborator invite, and the load-bearing `priority:high` / `priority:normal` / `priority:low` opener-routing labels. The dispatch is dry-run by default; execute mode performs cross-repository settings and label writes through `scripts/bootstrap_consumer_settings.py`. | ⚪ Manual | [Bootstrap consumer runs](https://github.com/stranske/Workflows/actions/workflows/maint-83-bootstrap-consumer.yml) |
...
The harvester's own recurring PR is not input evidence. `source_context.js`
recognizes it only when the head branch is `verifier-corpus-harvest/auto`, both
repositories are `stranske/Workflows`, and the `automation` plus
`model-selection` labels are present. `agents_verifier_context.js` then exits
