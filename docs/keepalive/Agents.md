...
3. **Gate is the trigger**: Keepalive is event-driven via Gate `workflow_run` completion. The root loop accepts only Gate runs whose source event was `pull_request`, so push-to-default-branch Gate runs are rejected at the job condition before a runner is allocated. A real PR Gate whose `pull_requests` association is empty remains eligible and is resolved by head SHA inside `evaluateKeepaliveLoop`. Auto-pilot's `monitor-pr` step watches for keepalive progress. Neither polls — both react to events.
