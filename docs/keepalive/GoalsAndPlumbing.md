...
- A possible access or authority boundary adds `agent:needs-attention` with an immediately due independent challenge. The hourly sweep reads the persisted challenge state and force-dispatches a current-state recheck; ordinary sweep traffic remains a non-forced re-evaluation. If that distinct recheck succeeds, it clears the automation-owned challenge. If it reproduces an authentication/permission failure, the summary records the confirmation and exact credential, scope, or permission action before replacing `agent:needs-attention` with `needs-human`. The reviewed-repo controller challenges confirmed holds again after 24 hours.
