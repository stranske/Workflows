...
- A possible access or authority boundary adds `agent:needs-attention` with an immediately due independent challenge. The persisted state includes a redacted fingerprint and runner-reported boundary detail. Every hourly sweep wakeup bypasses both state and completed-runner debounce so an unchanged zero-commit round is actually re-evaluated, but it force-dispatches only for a due claim carrying that exact fingerprint in an HMAC signed with the keepalive/Workflows App private key and bound to the repository, PR, random nonce, and exact sweep run/attempt. Generic retries and other workflows sharing `github-actions[bot]` cannot confirm the challenge; missing or invalid signing material fails closed to an ordinary non-forced recheck. A green recheck clears the challenge. Only the signed sweep-selected fingerprint failing again may record the confirmation and exact runner-reported credential, scope, or permission action before replacing `agent:needs-attention` with `needs-human`; a different auth failure starts a new automation-owned challenge that the next sweep can evaluate. The reviewed-repo controller challenges confirmed holds again after 24 hours.
