...
- **Budget exhaustion:** When the current iteration reaches `max_iterations`, keepalive stops that dispatch strategy, records reason `round-budget-exhausted`, and dispatches one forced recovery lease. The durable state records that lease as issued and consumes it only after an agent execution returns a real result; skipped or cancelled runners and temporary operator guards defer the same lease for reuse after the guard clears. Later ordinary events cannot mint a second lease for the same terminal reason and budget. Raising the budget or making later ordinary progress resets that boundary; the forced run itself may cross the persisted budget once and cannot recursively dispatch itself.
...
- Every non-transient run/fix failure records automation-owned retry state and explicitly dispatches a bounded retry while below the failure threshold. `agent:retry` is reserved as a fallback only when that dispatch fails, preventing a label event from racing the direct dispatch.
