...
- A possible access or authority boundary adds `agent:needs-attention` with an immediately due independent challenge. The persisted state includes a redacted fingerprint and runner-reported boundary detail. The hourly sweep force-dispatches a current-state recheck only for that due claim; ordinary sweep traffic remains non-forced. A green recheck clears the challenge. Only the same fingerprint failing again may record the confirmation and exact runner-reported credential, scope, or permission action before replacing `agent:needs-attention` with `needs-human`; a different auth failure starts a new automation-owned challenge. The reviewed-repo controller challenges confirmed holds again after 24 hours.
