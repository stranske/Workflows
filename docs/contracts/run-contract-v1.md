...
| Subfield | Req | Meaning |
| --- | --- | --- |
| `outputs.manifest_ref` | required | Reference to the run's `artifact-manifest/v1` manifest (`artifact:manifest.json` or a traversal-safe run-dir-relative POSIX path). Absolute paths, traversal, backslashes, drive roots, URLs, and empty path segments are rejected. The named artifacts live there, not inline. |
