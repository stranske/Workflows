...
- **`agents-autofix-dispatcher.yml`** — Triggered when Gate posts `autofix_gate_failure`. After consolidation it only validates the payload and records that `agents-81-gate-followups.yml` owns the repair path, so it needs no checkout, App token, or write permissions.
- **`agents-autofix-loop.yml`** — Legacy autofix loop retained where still present for larger fix-ups (pyproject sync, scripted rewrites, merge conflict handling). Consolidated consumers use `agents-81-gate-followups.yml` for Gate-driven repair routing, and failures still surface actionable labels/comments (`autofix:conflict`, `needs-human`) when automation cannot complete the repair.
...
- **`agents-autofix-dispatcher.yml`** — See Agents section; acknowledges legacy Gate autofix dispatch events while consolidated followups handle repair work.
...
3. **Consolidated followups** — `agents-autofix-dispatcher.yml` only acknowledges legacy `autofix_gate_failure` dispatches. `agents-81-gate-followups.yml` handles Gate `workflow_run` repair decisions, including rerouting to the autofix path and surfacing actionable labels/comments (`autofix:conflict`, `needs-human`) when automation cannot complete the repair.
