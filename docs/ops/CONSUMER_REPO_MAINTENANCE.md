...
Created and refreshed deliveries also carry `workflow:source-sync` and the
`workflow-source:sync_campaign` marker. Consumer event handlers treat generated
sync provenance as authoritative over issue-like text in a file summary only
when the source label, stable `sync/workflows-candidate` or
`sync/workflows-delivery` branch, and complete immutable consumer-sync marker
all agree. Removing any binding restores ordinary explicit-issue precedence.
