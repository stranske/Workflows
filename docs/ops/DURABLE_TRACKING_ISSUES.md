...
| [#499](https://github.com/stranske/Counter_Risk/issues/499) | Counter_Risk agent metrics weekly summary | [`agents-weekly-metrics.yml`](https://github.com/stranske/Counter_Risk/blob/main/.github/workflows/agents-weekly-metrics.yml) | Mondays 06:00 UTC | New comment per run |
| [#330](https://github.com/stranske/Inv-Man-Intake/issues/330) | Inv-Man-Intake agent metrics weekly summary | [`agents-weekly-metrics.yml`](https://github.com/stranske/Inv-Man-Intake/blob/main/.github/workflows/agents-weekly-metrics.yml) | Mondays 06:00 UTC | New comment per run |
| [#343](https://github.com/stranske/Pension-Data/issues/343) | Pension-Data agent metrics weekly summary | [`agents-weekly-metrics.yml`](https://github.com/stranske/Pension-Data/blob/main/.github/workflows/agents-weekly-metrics.yml) | Mondays 06:00 UTC | New comment per run |
...
- **Consumer metrics trackers (#499, #330, #343)** — Health 71 checks each consumer's scheduled `agents-weekly-metrics.yml` independently. A tracker comment begins with its last successful write timestamp; an `action_required` zero-job history is reported loudly with the documented web-UI approval remedy instead of being mistaken for a quiet week. Editing the workflow can re-arm GitHub's suspicious-workflow protection, so operators must approve the held run again after a template sync.
