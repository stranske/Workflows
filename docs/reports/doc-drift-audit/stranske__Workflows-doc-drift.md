...
|---|---|---|---|---|---|
| 1 | `README.md` | stale, covered by [#2085](https://github.com/stranske/Workflows/issues/2085) | Line 77 `verify:compare` model identifiers | `tools/langchain_client.py:96-98` (`_default_slots`); `tests/docs/test_workflow_source_docs.py::test_readme_verify_compare_models_match_langchain_defaults` | covered by #2085 (PR #2092 merged 2026-05-13) |
...
| 3 | `docs/ops/REPO_REVIEW_PROCESS.md` | stale, covered by [#2088](https://github.com/stranske/Workflows/issues/2088) | "Weekly Run" Phase-4 entry-point command at lines 52, 65, 71, 89 | `scripts/repo_review_coordinator.py` (present; Phase-4 entry point) and `scripts/repo_review_evaluator.py` (present; preflight). The doc currently names the coordinator as the primary entry point with the evaluator preserved for preflight use — fix from #2088 / PR #2095 (merged 2026-05-14T03:00:45Z) is on `main`. | covered by #2088 (PR #2095 merged 2026-05-14) |
| 4 | `docs/keepalive/GoalsAndPlumbing.md` | accurate | Agent table row 143: `agent:codex` → "Codex CLI (gpt-5.3-codex) → `reusable-codex-run.yml`" | `.github/workflows/reusable-codex-run.yml:63` (`default: 'gpt-5.3-codex'`); `.github/agents/registry.yml` entry for `codex` (runner = `reusable-codex-run.yml`). Other workflow references (Gate, keepalive loop, branch-sync) match `.github/workflows/`. | none required |
| 5 | `docs/AGENTS_POLICY.md` | contradictory | "Covered files" lists 2 workflows (`agents-63-issue-intake.yml`, `agents-70-orchestrator.yml`); "Verification checklist" later asserts "the three workflows in its 'Protected file patterns' section". | `.github/workflows/agents-guard.yml` exists and is named in "Protection layers" item 3 as the guard, but is not in "Covered files"; `.github/scripts/agents-guard.js:12` `DEFAULT_PROTECTED_PATHS = ['.github/workflows/agents-*.yml']` is broader than the doc's curated subset, so the "three" count is ambiguous unless the doc explicitly names the third workflow. | [#2098](https://github.com/stranske/Workflows/issues/2098) |
...
| 8 | `docs/ci/WORKFLOWS.md` | contradictory, covered by [#2085](https://github.com/stranske/Workflows/issues/2085) | Lines 19 (Gate dispatches `autofix_gate_failure` → `agents-autofix-dispatcher.yml`) vs prior 135 wording — described in #2089 as an autofix-dispatch contradiction. Current `main` text at line 19 names the dispatch path consistent with `.github/workflows/pr-00-gate.yml` and `.github/workflows/agents-autofix-dispatcher.yml:3`. | `.github/workflows/pr-00-gate.yml` (Gate; current head names the autofix dispatch event); `.github/workflows/agents-autofix-dispatcher.yml:3` (`on: repository_dispatch` for `autofix_gate_failure`); `tests/docs/test_workflow_source_docs.py::test_workflows_doc_names_gate_autofix_dispatch_path` is the regression guard. | covered by #2085 (PR #2092 merged 2026-05-13) |
...
- 4 classified `stale` or `contradictory`:
  - 2 covered by already-filed candidates from prior repo-review weekly runs:
    - `README.md` (stale, covered by [#2085](https://github.com/stranske/Workflows/issues/2085))
    - `docs/ci/WORKFLOWS.md` (contradictory, covered by [#2085](https://github.com/stranske/Workflows/issues/2085))
    - `docs/ops/REPO_REVIEW_PROCESS.md` (stale, covered by [#2088](https://github.com/stranske/Workflows/issues/2088))
  - 2 newly filed by this audit:
    - `docs/AGENTS_POLICY.md` (contradictory) → [#2098](https://github.com/stranske/Workflows/issues/2098)
    - `docs/LABELS.md` (stale) → [#2099](https://github.com/stranske/Workflows/issues/2099)
- 9 classified `accurate`: `docs/INTEGRATION_GUIDE.md`, `docs/keepalive/GoalsAndPlumbing.md`, `docs/keepalive/Agents.md`, `docs/MODEL_MANAGEMENT.md`, `docs/WORKFLOW_GUIDE.md`, `docs/ops/REPO_REVIEW_ROUND2_PROTOCOL.md`, `docs/ops/REPO_REVIEW_ROUND1_SCHEMA.md`, `AGENTS.md`. (Counting `docs/ops/REPO_REVIEW_PROCESS.md`'s post-#2088 state as `stale, covered by`; counting `README.md`'s post-#2085 state as `stale, covered by`.)
...
Internal contradiction was caught by reading each doc end-to-end against itself and against its named authoritative sources; the contradiction in `docs/AGENTS_POLICY.md` between "Covered files" (2 named) and the Verification checklist ("three workflows") is one such case.
...
No additional issues were filed for the already-covered instances (#2085, #2088) since their PRs are merged on `main`.
