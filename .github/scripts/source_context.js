...
function hasExplicitIssueReferencePrefix(value) {
  ...
  return /\b(?:close[sd]?|closing|fix(?:e[sd])?|fixing|resolve[sd]?|resolving|address(?:e[sd])?|addressing|relate[sd]?\s+to(?:\s+(?:issue|source\s+issue|github\s+issue))?|refs?(?:\s+(?:issue|source\s+issue|github\s+issue))?|references?(?:\s+(?:issue|source\s+issue|github\s+issue))?|issue|source\s+issue|github\s+issue)\s*[:#-]?\s*$/i.test(
    prefix
  );
}
